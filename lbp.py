...
def get_lbphs(img_paths):
    ...
    for img_path in img_paths:
        ...
        hist_dct = cv2.dct(hist.astype(np.float32)).T.take([0,1,2,3,4,5,6,7], axis=1)[0]
        # print(hist_dct.shape)
        lbphs.append(hist_dct)
    return lbphs

def train_svc(X_train, y_train):
    clf = SVR(C=1e3, kernel='rbf', gamma=0.1)
    print("[INFO] 开始训练... ")
    # clf.fit(get_lbphs(X_train), y_train)
    score = OneVsRestClassifier(clf, n_jobs=-1).fit(get_lbphs(X_train), y_train)
    joblib.dump(score, "./output/lbp/lbp_clf.model")
